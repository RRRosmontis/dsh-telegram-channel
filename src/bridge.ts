...
interface TgPendingAsk {
  ...
  ask: TgAnswerHandle
  at: number
  /** question index → option indices chosen via inline-keyboard taps. */
...
export class TelegramBridge {
    ...
    if (quoted) {
      try {
        await this.withRetry(() =>
          this.client.sendMessage(chatId, `<blockquote>${escapeHtml(body)}</blockquote>`, 'HTML'),
        )
        return
      } catch (err) {
        this.ctx.logger.warn(
          `dsh-telegram-channel: blockquote notice failed (${this.redact(err)}), falling back to plain text`,
        )
      }
    }
    try {
  ...
  private installAskInterceptor(): void {
    if (this.askInstalled) return
    let svc: { constructor: { prototype: Record<string | symbol, unknown> } } | undefined
    try {
      svc = (this.ctx as unknown as { get: (name: string) => unknown }).get('userQuestions') as
    ...
    const prev = proto[kTgAskWrapped] as { restore: () => void; bridge: unknown } | undefined
    if (prev?.bridge === this) {
    ...
    const orig = proto.ask as (this: unknown, request: unknown) => Promise<unknown>
    const bridge = this
    const wrapped = function (this: unknown, request: unknown): Promise<unknown> {
      return bridge.aroundUserQuestionAsk(this, request, orig)
    }
    proto.ask = wrapped
    const restore = (): void => {
      if (proto.ask === wrapped) proto.ask = orig
    }
  ...
  private aroundUserQuestionAsk(
    service: unknown,
    request: unknown,
    orig: (this: unknown, request: unknown) => Promise<unknown>,
  ): Promise<unknown> {
  ...
  private async deliverAskPrompt(chatId: number, pending: TgPendingAsk): Promise<void> {
    const { questions } = pending
    try {
      await this.withRetry(() =>
        this.client.sendMessage(
          chatId,
          `❓ 即将向你提问（共 ${questions.length} 题）——点按钮或回复文本作答；/cancel 关闭 TG 作答`,
        ),
      )
      for (let i = 0; i < questions.length; i++) {
        ...
        if (opts.length === 0) {
          await this.withRetry(() =>
            this.client.sendMessage(chatId, `${header}${q.question ?? ''}\n（开放问题，请直接回复文本）`),
          )
          continue
        }
        const keyboard: InlineKeyboardMarkup = {
  ...
  private finalizeAskIfComplete(chatId: number, pending: TgPendingAsk): boolean {
  ...
  private async handleAskCallback(chatId: number, cq: { id: string; data?: string }): Promise<void> {
    const pending = this.pendingAsks.get(String(chatId))
    if (!pending) {
    ...
    if (action === 'ok') {
      ...
      if (q.multiSelect) {
        const set = pending.kbSel.get(qi) ?? new Set<number>()
