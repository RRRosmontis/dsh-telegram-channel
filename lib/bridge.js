...
import { escapeHtml, markdownToHtml, splitMessage } from './format.js';
...
/** Cap a single reasoning block so a long chain of thought can't flood the phone. */
const REASONING_QUOTE_MAX_CHARS = 800;
/**
...
export class TelegramBridge {
    ...
    start() {
        ...
        this.loadBindings();
        // ask_user interception: see installAskInterceptor — a prototype wrap of
        // UserQuestionService.prototype.ask. Plain ctx.on('user-questions/request')
        // registries never see the dispatch (the waterfall fires on a different
    ...
    async processUpdate(update) {
        ...
        switch (parsed.type) {
            ...
            case 'compact':
                ...
                return;
            case 'goal':
                await this.requestGoal(chatId, parsed.arg);
                return;
            case 'rich':
    ...
    async onSessionEvent(session, event) {
        if (!this.askInstalled)
            this.installAskInterceptor();
        if (!this.approvalInstalled)
            this.installApprovalInterceptor();
        ...
        if (eventType === 'goal/changed') {
            ...
            if (phase === 'blocked' || phase === 'complete') {
                ...
                const label = phase === 'blocked'
                    ...
                    : '目标已达成 ✓';
                for (const b of targets)
                    this.enqueueNotice(b.chatId, `> ${label}`);
            }
            return;
        }
    ...
    async deliverNotice(chatId, text) {
        ...
        if (quoted) {
            try {
                await this.withRetry(() => this.client.sendMessage(chatId, `<blockquote>${escapeHtml(body)}</blockquote>`, 'HTML'));
                return;
            }
            catch (err) {
                this.ctx.logger.warn(`dsh-telegram-channel: blockquote notice failed (${this.redact(err)}), falling back to plain text`);
            }
        }
        try {
            await this.withRetry(() => this.client.sendMessage(chatId, body));
        }
        catch (err) {
            this.ctx.logger.error(this.redact(err));
        }
    ...
    // ── ask_user_question: TG answering via a prototype interceptor ──
    //
    // dsh 0.1.5-rc.1 routes the 'user-questions/request' waterfall through a
    // per-fiber events registry: plain ctx.on listeners in other fibers (this
    // plugin included) never see the dispatch, so the waterfall hook cannot
    // mirror questions to TG. The class is shared across instances though, so
    // wrapping UserQuestionService.prototype.ask intercepts EVERY ask with its
    // agent + questions, regardless of fiber. The wrap races the TG answer
    // against the original ask (GUI path), preserving the old dual-path
    // semantics. Restore-on-stop keeps hot reloads from stacking wrappers.
    installAskInterceptor() {
        if (this.askInstalled)
            return;
        let svc;
        try {
            svc = this.ctx.get('userQuestions');
        }
        catch {
        ...
        const prev = proto[kTgAskWrapped];
        if (prev?.bridge === this) {
            this.askInstalled = true;
            this.askRestore = prev.restore;
            return;
        }
        prev?.restore(); // stale wrapper from a disposed bridge instance
        const orig = proto.ask;
        const bridge = this;
        const wrapped = function (request) {
            return bridge.aroundUserQuestionAsk(this, request, orig);
        };
        proto.ask = wrapped;
        const restore = () => {
            if (proto.ask === wrapped)
                proto.ask = orig;
        };
        proto[kTgAskWrapped] = { restore, bridge: this };
        this.askRestore = restore;
        this.askInstalled = true;
    }
    aroundUserQuestionAsk(service, request, orig) {
        const tg = this.registerTgAsk(request);
        let gui;
        try {
            gui = Promise.resolve().then(() => orig.call(service, request));
        }
        catch (err) {
            tg.reject(err);
            throw err;
        }
        void gui.then(() => this.settleGuiSide(request), () => this.settleGuiSide(request));
    ...
    async deliverAskPrompt(chatId, pending) {
        const { questions } = pending;
        try {
            await this.withRetry(() => this.client.sendMessage(chatId, `❓ 即将向你提问（共 ${questions.length} 题）——点按钮或回复文本作答；/cancel 关闭 TG 作答`));
            for (let i = 0; i < questions.length; i++) {
                const q = questions[i];
                const opts = Array.isArray(q.options) ? q.options : [];
                const header = questions.length > 1 ? `Q${i + 1}. ` : '';
                if (opts.length === 0) {
                    await this.withRetry(() => this.client.sendMessage(chatId, `${header}${q.question ?? ''}\n（开放问题，请直接回复文本）`));
                    continue;
                }
                const keyboard = {
                    inline_keyboard: [
                ...
                await this.withRetry(() => this.client.sendMessage(chatId, `${header}${q.question ?? ''}`, undefined, keyboard));
            }
        }
        catch (err) {
            this.ctx.logger.error(this.redact(err));
    ...
    finalizeAskIfComplete(chatId, pending) {
        const { questions, ask } = pending;
        const answers = [];
        for (let i = 0; i < questions.length; i++) {
            const q = questions[i];
            const opts = Array.isArray(q.options) ? q.options : [];
            const sel = pending.kbSel.get(i);
            if (sel && sel.size > 0) {
                answers.push({ id: q.id, selected: [...sel].sort((a, b) => a - b).map((j) => opts[j].label) });
                continue;
            }
            const text = pending.textAnswers.get(i);
            if (text !== undefined) {
                answers.push(text);
                continue;
            }
    ...
    async handleTgAnswer(chatId, pending, text) {
        ...
        for (const [idx, raw] of parts) {
            ...
            if (isLetterChoice) {
                ...
                pending.textAnswers.set(idx, { id: q.id, selected });
            }
            else {
        ...
        if (!this.finalizeAskIfComplete(chatId, pending)) {
            const done = questions.filter((_, i) => pending.kbSel.get(i)?.size || pending.textAnswers.has(i)).length;
            await this.enqueueNotice(chatId, `已记录 ${done}/${questions.length} 题，请继续作答（按钮或 1:/2: 文本）`);
        }
    }
    /** Inline-keyboard tap on an ask message: toggle/record the choice, resolve when every question is answered. */
    async handleAskCallback(chatId, cq) {
        const pending = this.pendingAsks.get(String(chatId));
        if (!pending) {
            await this.client.answerCallbackQuery(cq.id, '该提问已失效（可能已在 Web 端作答）');
            return;
        }
        const payload = (cq.data ?? '').slice(ASK_CB.length);
        const sep = payload.indexOf(':');
        const qi = Number(payload.slice(0, sep));
        const action = payload.slice(sep + 1);
        const q = pending.questions[qi];
        if (!q) {
            await this.client.answerCallbackQuery(cq.id, '题目已失效');
            return;
        }
        const opts = Array.isArray(q.options) ? q.options : [];
        if (action === 'custom') {
        ...
        if (action === 'ok') {
            ...
            const j = Number(action);
            if (!Number.isInteger(j) || j < 0 || j >= opts.length) {
                await this.client.answerCallbackQuery(cq.id, '选项不存在');
                return;
            }
            if (q.multiSelect) {
                const set = pending.kbSel.get(qi) ?? new Set();
                if (set.has(j))
                    set.delete(j);
                else
                    set.add(j);
                pending.kbSel.set(qi, set);
                await this.client.answerCallbackQuery(cq.id, set.size > 0 ? `已选 ${set.size} 项，选完点"提交所选"` : '已取消该选项');
            }
            else {
                pending.kbSel.set(qi, new Set([j]));
                await this.client.answerCallbackQuery(cq.id, `已选：${opts[j].label}`);
            }
        }
        if (!this.finalizeAskIfComplete(chatId, pending) && action === 'ok') {
            await this.client.answerCallbackQuery(cq.id, '已提交本题选择，请继续作答其他问题');
        }
    }
    ...
    installApprovalInterceptor() {
        ...
        if (!svc)
            return;
        const proto = svc.constructor.prototype;
        const prev = proto[kTgApprovalWrapped];
        if (prev?.bridge === this) {
        ...
        const wrapped = function (req) {
            return bridge.aroundApprovalRequest(this, req, orig);
        };
        proto.request = wrapped;
        const restore = () => {
            if (proto.request === wrapped)
                proto.request = orig;
        };
    ...
    aroundApprovalRequest(service, req, orig) {
        const sessionId = req?.agent?.id !== undefined ? String(req.agent.id) : undefined;
    ...
    async deliverApprovalPrompt(chatId, toolName, reason) {
        const keyboard = {
            inline_keyboard: [[
        ...
        try {
            await this.withRetry(() => this.client.sendMessage(chatId, `权限审批：${toolName}${reason ? `\n(${reason})` : ''}`, undefined, keyboard));
        }
        catch (err) {
            this.ctx.logger.error(this.redact(err));
    ...
    async handleApprovalCallback(chatId, cq, data) {
        const approval = this.pendingApprovalsTG.get(String(chatId));
        if (!approval) {
